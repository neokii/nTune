...
class SshSession (val host: String, val port: Int) {
    ...
    init
    {
        jsch.addIdentity("id_rsa", privateKey.toByteArray(), publicKey.toByteArray(), "passphrase".toByteArray())
    }

    interface OnConnectListener { fun onConnect() fun onFail(e:Exception) }
    interface OnResponseListener { fun onResponse(res:String) fun onFail(e:Exception) }
    ...
    fun send(command: String, listener: OnResponseListener?)
    {
        Executors.newSingleThreadExecutor().execute {
    ...
    private fun waitUntilChannelClosed(executionChannel: ChannelExec) {
